...
function parseAuthor({ fields }) {
  return {
    ...
    picture2: fields.picture2 ? fields.picture2.fields.file : { url: null },
  };
}
...
function parsePost({ sys, fields }) {
  let coverImage;
  if (fields.image) {
    coverImage = { url: fields.image[0].fields.file.url };
  } else {
    coverImage = { url: parseFallbackImage(fields.fallbackImageUrl) };
  }
  return {
    id: sys.id,
    title: fields.title,
...
export async function getAuthor(preview, entryId) {
  const entry = await getClient(preview).getEntry(entryId);
  return parseAuthor(entry);
}
